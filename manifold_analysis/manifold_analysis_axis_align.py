...
def manifold_analysis(XtotT, kappa, n_t, t_vecs=None):
    ...
    for i in range(num_manifolds):
        ...
        # Compute the SVD of the reduced data
        svd_i = np.linalg.svd(S_r)
        svd_results.append(svd_i)
    ...
    cent_cent = compute_cent_cent(np.array(means).squeeze())
    ax_ax = compute_ax_ax(svd_results)
        
    return a_Mfull_vec, R_M_vec, D_M_vec, cent_cent, ax_ax


# Compute center correlations
def compute_cent_cent(centers):
    # Calculate the norms of the center vectors
    norms = np.linalg.norm(centers, axis=1, keepdims=True)
    # Use broadcasting to compute the inner product matrix and normalize
    inner_product = np.abs(np.dot(centers, centers.T))
    normalized_corr = inner_product / (norms @ norms.T)
    
    # Exclude diagonal elements (i.e., the case where μ = ν)
    np.fill_diagonal(normalized_corr, 0)
    
    return normalized_corr # np.mean(normalized_corr[normalized_corr > 0])  # Compute the mean of non-zero correlations

# 2024-11-1 TODO: debug this function
# Compute axis correlations
def compute_ax_ax(svd_results):

    num_manifolds = len(svd_results)
    
    # Extract left singular vectors u and singular values λ from SVD results
    u_matrices = np.array([svd[0] for svd in svd_results])
    lambda_matrices = np.array([svd[1] for svd in svd_results])
    
    # Compute normalized singular values
    hat_lambda_matrices = lambda_matrices / np.sqrt(np.sum(lambda_matrices**2, axis=1, keepdims=True))

    print('check shape of u_matrices, lambda_matrices, hat_lambda_matrices:', u_matrices.shape, lambda_matrices.shape, hat_lambda_matrices.shape)
    
    # method 3
    ax_ax_corr = np.zeros((num_manifolds, num_manifolds))
    for mu in range(num_manifolds):
        for nu in range(mu + 1, num_manifolds):
            inner_product = np.abs(np.tensordot(u_matrices[mu], u_matrices[nu].T, axes=([-1], [-1])))
            print('check shape of inner_product:', inner_product.shape)
            axis_correlation = np.sum(np.diag(hat_lambda_matrices[mu] * hat_lambda_matrices[nu].T) * np.diag(inner_product))
            # axis_correlation = np.sum(hat_lambda_matrices[mu] * hat_lambda_matrices[nu].T * inner_product)
            ax_ax_corr[mu, nu] = axis_correlation
    # 2024-11-7 remove normalization
    normalized_ax_ax_corr = 2 * ax_ax_corr # / (num_manifolds * (num_manifolds - 1))
    # # Use broadcasting to compute axis correlations
    
    # method 1
    # u_matrices_t = np.transpose(u_matrices, (0, 2, 1))  # (num_manifolds, num_features, num_components)
    # ax_ax_corr = np.einsum('ik,jk->ij', hat_lambda_matrices, hat_lambda_matrices) * np.abs(np.einsum('ijk,ikl->ij', u_matrices, u_matrices_t))

    # method 2
    # # Compute the inner product between u matrices
    # inner_product = np.abs(np.einsum('ijk,ijk->ij', u_matrices, u_matrices))
    
    # # Calculate the axis correlations
    # ax_ax_corr = np.einsum('ij,ik->jk', hat_lambda_matrices, hat_lambda_matrices) * inner_product
    
    # np.fill_diagonal(ax_ax_corr, 0)
    
    # Compute the mean of non-zero correlations
    return normalized_ax_ax_corr # np.mean(ax_ax_corr[ax_ax_corr > 0])
